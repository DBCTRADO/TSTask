...
namespace TSTask
{
	...
	bool CIniFile::Open(LPCWSTR pszFileName,UINT Flags)
	{
		...
		OutLog(LOG_VERBOSE,L"INI ファイル \"%s\" を開きます。",pszFileName);

		String MutexName;
		MutexName=L"TSTask_Ini_Mutex_";
		MutexName+=pszFileName;
		StringUtility::Replace(MutexName,L'\\',L':');
		if (m_Mutex.Create(MutexName.c_str())) {
			if (m_Mutex.Wait(5000)==WAIT_TIMEOUT) {
				OutLog(LOG_ERROR,L"INI ファイル \"%s\" がロックされたままになっています。",pszFileName);
				m_Mutex.Close();
				//return false;
			}
		} else {
			OutLog(LOG_ERROR,L"INI ファイルの Mutex \"%s\" が作成できません。",MutexName.c_str());
		}
	...
	bool CIniFile::Close()
	{
		...
		if ((m_OpenFlags&OPEN_WRITE)!=0 && m_hFile!=INVALID_HANDLE_VALUE) {
			...
			DWORD DataSize=DWORD(Buffer.size()*sizeof(String::value_type));
			if (!::WriteFile(m_hFile,&BOM,sizeof(BOM),&Write,nullptr) || Write!=sizeof(BOM)
					|| !::WriteFile(m_hFile,Buffer.data(),DataSize,&Write,nullptr)
					|| Write!=DataSize) {
				fOK=false;
			} else {
	...
	bool CIniFile::DeleteValues(LPCWSTR pszMask)
	{
		if ((m_OpenFlags & OPEN_WRITE)==0
				|| m_Section.empty()
				|| IsStringEmpty(pszMask))
			return false;
		...
		for (auto itrValue=pSection->Entries.begin();itrValue!=pSection->Entries.end();) {
			if (!itrValue->Name.empty() && IsMatchWildcard(itrValue->Name.c_str(),pszMask)) {
				itrValue=pSection->Entries.erase(itrValue);
				fDeleted=true;
			} else {
				itrValue++;
			}
